...
describe('FactExtractor', () => {
  ...
  describe('Fact Extraction', () => {
    test('should extract facts from conversation messages', async () => {
      ...
      const messages: ConversationMessage[] = [
        ...
        },
        {
          ...
          conversation_id: conversationId,
          role: 'assistant',
          content: 'That\'s a great combination! React and TypeScript work really well together.',
          timestamp: new Date().toISOString(),
          processed_for_facts: false
        },
        {
    ...
    test('should handle empty messages array', async () => {
      ...
      await expect(factExtractor.extractFacts([], conversationId))
    ...
    test('should handle AI service errors gracefully', async () => {
      ...
      mockAIService.processPrompt = jest.fn().mockRejectedValue(new Error('AI service error'));

      await expect(factExtractor.extractFacts(messages, conversationId))
        .resolves.not.toThrow();
    ...
    test('should handle invalid JSON response from AI', async () => {
      ...
      mockAIService.processPrompt = jest.fn().mockResolvedValue('invalid json response');
    ...
    test('should skip facts with missing required fields', async () => {
      ...
      mockAIService.processPrompt = jest.fn().mockResolvedValue(JSON.stringify([
        {
          content: 'Valid fact',
          category: 'preferences'
        },
  ...
  describe('Message Processing', () => {
    test('should mark messages as processed', async () => {
      ...
      await memoryService.createConversation('Test Conversation');
      const message1 = await memoryService.addMessageToConversation(conversationId, 'user', 'Test message 1');
      const message2 = await memoryService.addMessageToConversation(conversationId, 'assistant', 'Test response 1');
      ...
      const updatedMessages = await memoryService.getMessagesForConversation(conversationId);
    ...
    test('should handle different message roles', async () => {
      ...
      expect(mockAIService.processPrompt).toHaveBeenCalledWith(
        expect.stringContaining('assistant: That sounds like an interesting career!')
      );
    });
  });
  ...
  describe('Categories', () => {
    test('should extract facts with correct categories', async () => {
      ...
      const messages: ConversationMessage[] = [{
        ...
        conversation_id: conversationId,
        role: 'user',
        content: 'I\'m a software engineer who loves hiking with my dog, and I always use dark mode',
        timestamp: new Date().toISOString(),
        processed_for_facts: false
      }];
